...
PLANOS = [
    ...
    {'id':5,'moedas':69,'preco':39.69,'nome':'👑 ULTRA','desc':'R$0,57/VOLT','tag':'69 ciclos','bonus':'🎨 1 Skin Lendária GRÁTIS','desconto':'69% OFF'},
    {
        ...
        'cor_botao': 'linear-gradient(135deg,#cc0000,#ffd700,#00aa00)', 'cor_tab_ativa': '#ffd700',
        'cor_header_bg': 'linear-gradient(135deg,#0a1a0a,#1a2a0a,#2a3a0a,#1a2a0a,#0a1a0a)', 'cor_header_borda': '#ffd700',
        'header_extra': '<div style="position:absolute;top:10px;left:20px;font-size:30px;z-index:1;animation:float 2s ease-in-out infinite">🦁</div><div style="position:absolute;top:10px;right:20px;font-size:30px;z-index:1;animation:float 2s ease-in-out infinite;animation-delay:1s">🎸</div>',
        'css_extra': 'body{background:linear-gradient(180deg,#0a1a0a 0%,#001a00 100%)!important}.header{border-color:#ffd700!important;box-shadow:0 0 30px rgba(255,215,0,0.3)}.header h1{background:linear-gradient(135deg,#00aa00,#ffd700,#cc0000)!important;-webkit-background-clip:text!important;-webkit-text-fill-color:transparent!important}.terminal{color:#ffd700!important;font-family:monospace!important}@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}'
    },
...
def admin_salvar():
    ...
    if 'estrategias' in d: u['estrategias_compradas'] = d['estrategias']
...
LOGIN_HTML = """<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Admin</title>
<style>body{background:#0a0a1a;display:flex;justify-content:center;align-items:center;height:100vh;font-family:monospace;margin:0}
.box{background:#1a1a3e;border:2px solid #ffd700;border-radius:15px;padding:25px;text-align:center;width:280px}
h2{color:#ffd700;margin:0 0 15px}input{padding:14px;background:#111;border:1px solid #333;color:#fff;border-radius:10px;font-size:18px;text-align:center;width:100%;box-sizing:border-box;margin:10px 0}
.btn{background:#ffd700;color:#000;padding:14px;border:none;border-radius:10px;font-weight:bold;cursor:pointer;font-size:16px;width:100%}</style></head>
<body><div class="box"><h2>🔐 Admin</h2><form method="POST"><input type="password" name="senha" placeholder="Senha" autofocus><br><button class="btn" type="submit">Entrar</button></form></div></body></html>"""

ADMIN_HTML = """<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Painel Admin</title>
<style>body{background:#0a0a1a;color:#fff;font-family:monospace;padding:10px;margin:0}.container{max-width:500px;margin:0 auto}h1{color:#ffd700;text-align:center;font-size:1.2em}.card{background:#1a1a3e;border:1px solid #ffd700;border-radius:12px;padding:15px;margin:10px 0}
...
.btn{background:#ffd700;color:#000;padding:14px;border:none;border-radius:10px;font-weight:bold;cursor:pointer;width:100%;margin:8px 0;font-size:16px}.btn-green{background:#00ff88}.btn-red{background:#ff4444;color:#fff}
.result{background:#000;padding:12px;border-radius:8px;margin:10px 0;white-space:pre-wrap;font-size:13px;max-height:200px;overflow-y:auto;word-break:break-all}</style></head>
<body><div class="container"><h1>🔐 Admin</h1>
<div class="card"><h3 style="color:#ffd700;margin:0 0 10px">🔍 Buscar</h3>
...
<div class="card"><h3 style="color:#ffd700;margin:0 0 10px">✏️ Editar</h3>
...
<script>
function buscar(){fetch('/api/admin/buscar?email='+document.getElementById('emailBusca').value).then(r=>r.json()).then(d=>{if(d.erro)document.getElementById('resultado').innerHTML='<p style=color:#ff4444>'+d.erro+'</p>';else{var h='<p style=color:#00ff88>✅ '+d.email+'</p>';h+='<p>⚡ '+d.moedas+' VOLTS | 💵 R$ '+d.lucro_total.toFixed(2)+'</p>';document.getElementById('resultado').innerHTML=h;document.getElementById('emailEdit').value=d.email;document.getElementById('moedas').value=d.moedas;document.getElementById('skin').value=d.skin_atual;document.getElementById('estrategias').value=d.estrategias_compradas.join(',')}})}
function salvar(){fetch('/api/admin/salvar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:document.getElementById('emailEdit').value,moedas:parseInt(document.getElementById('moedas').value),skin:document.getElementById('skin').value,estrategias:document.getElementById('estrategias').value.split(',').map(s=>s.trim())})}).then(r=>r.json()).then(d=>{document.getElementById('resultadoEdit').innerHTML='<p style=color:'+(d.ok?'#00ff88':'#ff4444')+'>'+d.msg+'</p>'})}
function resetar(){fetch('/api/admin/resetar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:document.getElementById('emailEdit').value})}).then(r=>r.json()).then(d=>{document.getElementById('resultadoEdit').innerHTML='<p style=color:'+(d.ok?'#00ff88':'#ff4444')+'>'+d.msg+'</p>'})}
</script></body></html>"""
