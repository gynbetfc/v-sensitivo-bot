...
def contar_dados_locais(db_id: str) -> Dict:
    try:
        with get_db_context() as conn:
            ...
            try:
                k = conn.execute("SELECT COUNT(*) FROM kits WHERE db_id=?", (db_id,)).fetchone()[0]
            except Exception:
                k = 0
            return {'produtos': p, 'clientes': c, 'vendas': v, 'kits': k, 'total': p + c + v + k}
    except:
        return {'produtos': 0, 'clientes': 0, 'vendas': 0, 'kits': 0, 'total': 0}
...
def _normalizar_dados_firebase(dados_fb: Dict) -> Dict:
    ...
    if vendas is None:
        ...
        resultado['vendas'] = []
    # KITS: preserva os kits (combos) vindos do Firebase. Sem isso, um kit criado
    # em outro dispositivo era DESCARTADO aqui e nunca chegava a este aparelho.
    kits = dados_fb.get('kits')
    if kits is None:
        resultado['kits'] = {}
    elif isinstance(kits, dict):
        resultado['kits'] = kits
    elif isinstance(kits, list):
        novo_dict = {}
        for item in kits:
            if isinstance(item, dict):
                kid = item.get('id') or str(uuid.uuid4())[:8]
                novo_dict[str(kid)] = item
        resultado['kits'] = novo_dict
    else:
        resultado['kits'] = {}
    # Preserva também as exclusões (tombstones), usadas para não ressuscitar itens
    if dados_fb.get('exclusoes') is not None:
        resultado['exclusoes'] = dados_fb.get('exclusoes')
    return resultado
...
def _baixar_firebase_para_local(db_id: str, dados_firebase: Dict, resultado: Dict) -> None:
    ...
    with get_db_context() as conn:
        ...
        # KITS: baixa os kits (combos) do Firebase, respeitando tombstones
        cur_exc_k = conn.execute("SELECT item_id FROM exclusoes WHERE tipo='kit' AND db_id=?", (db_id,))
        kits_excluidos = {str(r[0]) for r in cur_exc_k.fetchall()}
        for kit_id, dados_kit in (dados_firebase.get('kits') or {}).items():
            try:
                if str(kit_id) in kits_excluidos:
                    continue
                itens_kit = dados_kit.get('itens', [])
                itens_json = itens_kit if isinstance(itens_kit, str) else json.dumps(itens_kit)
                conn.execute("""INSERT OR IGNORE INTO kits (id, nome, preco, itens, db_id, ativo, ultima_atualizacao)
                    VALUES (?, ?, ?, ?, ?, ?, ?)""", (kit_id, dados_kit.get('nome', ''),
                    dados_kit.get('preco', 0), itens_json, db_id,
                    1 if dados_kit.get('ativo', True) else 0,
                    dados_kit.get('ultima_atualizacao', get_timestamp())))
            except Exception as e:
...
def login():
    try:
        ...
        def _fazer_login(user_db_id, user_id, nome, cargo, servidor_id_val, nome_loja_val, cnpj_val, cnpj_dados_val):
            ...
            # Sessão de NAVEGAÇÃO: o cookie de login morre quando a janela (webview)
            # fecha. Assim, ao reabrir o programa, cai na tela de login em vez de
            # entrar sozinho. O usuário salvo (localStorage) e as preferências
            # continuam guardados — só o login automático é impedido.
...
if __name__ == '__main__':
    ...
    # O LAUNCHER já baixa a interface (index.html) do GitHub antes de subir o
    # servidor. Então aqui NÃO baixamos de forma síncrona (era isso que travava
    # a tela de "Carregando" quando a internet estava lenta). O servidor sobe na
    # hora, usando o index.html que já está na pasta de templates.
    #
    # Caso alguém rode o pdv.py SEM o launcher (uso avançado/teste) e não exista
    # nenhum HTML local, aí sim baixamos uma vez (com timeout curto) para o
    # sistema não abrir sem tela. Se existir HTML local, servimos na hora e
    # atualizamos em segundo plano, sem travar.
    caminho_html_local = os.path.join(TEMPLATES_DIR, "index.html")
    if not os.path.exists(caminho_html_local):
        # Primeira vez e sem launcher: precisa baixar ao menos uma vez
        logger.warning("⚠️ Sem HTML local. Baixando uma vez para poder abrir...")
        baixar_html_github()
    else:
        # Já existe HTML: sobe na hora e atualiza em segundo plano (não trava)
        def _atualizar_html_bg():
            try:
                baixar_html_github()
            except Exception:
                pass
        threading.Thread(target=_atualizar_html_bg, daemon=True).start()
