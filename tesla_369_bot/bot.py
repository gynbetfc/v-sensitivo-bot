...
# ⚡ TESLA 369 BOT v15.8.0 - ENGINE PURA CONTEXTUAL ⚡
# Firebase: SKINS e ESTRATÉGIAS carregadas da nuvem
# FIX: Desconexão mata o servidor, versão dinâmica no HTML, e gales sem delay de relógio
...
print(f"✅ Firebase HTTP REST configurado! Versão: {BOT_VERSION}")
...
def carregar_e_injetar_estrategia(nome_estrategia):
    ...
    agora = time.time()
    if cache_estrategia_carregada["nome"] == nome_estrategia and cache_estrategia_carregada["codigo"] is not None: return True
...
def executar_ciclo(direcao):
    ...
    try:
        ...
        for i in range(MARTINGALE + 1):
            ...
            # 🔥 REQUISITO 4: SE FOR GALE (i > 0), NÃO AGUARDA VELAR, ENTRA RETO!
            if i == 0:
            ...
            # Puxa profile para estourar o cache do saldo
            try: API.get_profile()
            except: pass
            ...
            if lucro_liquido > 0:
                ...
                if u:
                    ...
                    u['banca_atual'] = round(saldo_depois, 2)
                    u.setdefault('historico_operacoes', []).append({'data': str(datetime.now())[:19], 'resultado': 'LOSS', 'valor': valor, 'lucro': lucro_liquido, 'estrategia': estrategia_atual_global.upper()})
                    salvar_usuario(email_usuario_atual, u)
            ...
            try:
                ...
                if velas and len(velas) >= 20:
                    ...
                    ultima_analise = {
                        ...
                        'stoch': round(estoc_val, 1), 'fase': "CONECTADO", 'preco': round(preco_atual, 5) if preco_atual else 0
                    }
            except: pass
        time.sleep(2)
...
def index():
    ...
    skin = next((s for s in skins if s.get('id') == skin_atual_global), skins[0] if skins else {'cor_fundo': '#0a0a1a', 'cor_panel': '#1a1a3e', 'cor_destaque': '#ffd700', 'cor_texto': '#fff', 'cor_botao': 'linear-gradient(135deg,#cc8800,#ffd700)', 'cor_tab_ativa': '#ffd700', 'cor_header_bg': '#1a0000', 'cor_header_borda': '#ffd700', 'css_extra': '', 'header_extra': ''})
    ...
    # REQUISITO 3: INJEÇÃO DA VARIÁVEL VERSÃO DO CORE PARA O JINJA2 NO LAYOUT
...
def selecionar_estrategia():
    ...
    if est_id not in estrategias_compradas:
        if not estrategias_info[est_id].get('gratis', False): return jsonify({'ok': False, 'erro': f'Estrategia bloqueada!'})
...
def parar():
    ...
    # 🔥 REQUISITO 1: SE O COMANDO FOR DESCONECTAR, MATA O PROCESSO DO FLASK IMEDIATAMENTE
    if data.get('desconectar'):
        conectado_iq = False
        add_log("🔌 Desconexao total requisitada. Desligando servidor...", "error")
        os.kill(os.getpid(), signal.SIGTERM)
        return jsonify({'ok': True, 'shutdown': True})
    
    add_log("✅ Bot parado pelo usuario!", 'win')
