...
# ============================================================
# FUNÇÃO PARA CARREGAR O HTML EXTERNO
# ============================================================

def carregar_html():
    """
    Carrega o arquivo index.html da mesma pasta do script.
    Procura em múltiplos locais para garantir que encontra.
    
    Retorna:
        str: Conteúdo do arquivo HTML
    """
    # Lista de possíveis caminhos para procurar o index.html
    possiveis_caminhos = []
    
    # 1. Mesma pasta do script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    possiveis_caminhos.append(os.path.join(script_dir, 'index.html'))
    
    # 2. Pasta atual de trabalho
    possiveis_caminhos.append(os.path.join(os.getcwd(), 'index.html'))
    
    # 3. Pasta pai do script
    possiveis_caminhos.append(os.path.join(os.path.dirname(script_dir), 'index.html'))
    
    # 4. Caminho relativo (fallback)
    possiveis_caminhos.append('index.html')
        ...
        if os.path.exists(caminho):
            try:
                with open(caminho, 'r', encoding='utf-8') as arquivo:
                    ...
                    add_log(f"✅ HTML carregado de: {caminho}", "info")
                    return conteudo
            except Exception as e:
                add_log(f"⚠️ Erro ao ler {caminho}: {e}", "error")
                continue
    
    # Se não encontrou em lugar nenhum
    add_log("❌ Arquivo index.html não encontrado! Verifique os caminhos:", "error")
    for caminho in possiveis_caminhos:
        add_log(f"   - {caminho}", "error")
    
    # Retorna HTML de emergência
    return """
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        <title>⚠️ ERRO - TESLA 369</title>
        <style>
            body{background:#0a0a1a;color:#fff;font-family:monospace;padding:20px}
            .erro{border:2px solid #ff4444;background:#1a0000;padding:20px;border-radius:10px}
            h1{color:#ff4444}
    ...
    <body>
        <div class="erro">
            ...
            <p>O arquivo <code>index.html</code> não foi encontrado em nenhum dos caminhos esperados.</p>
            <p>Certifique-se de que o arquivo existe na mesma pasta do <code>bot.py</code>.</p>
            <hr>
            <p><strong>Caminhos procurados:</strong></p>
            <ul>
    """ + "".join([f"<li><code>{caminho}</code></li>" for caminho in possiveis_caminhos]) + """
            </ul>
        </div>
    </body>
    </html>
...
def processar_html_com_skin():
    """
    Carrega o HTML do arquivo externo e aplica as cores da skin atual.
    
    Retorna:
        str: HTML processado com as cores da skin
    """
    # Carrega o HTML base do arquivo externo
